...
public interface RecordEventsListener {
    /**
     * Notified whenever an error has occurred on the record, usually due to an async operation such
     * as a timeout or VersionConflict that can't be caught sync.
     *
     * @param recordName   The name of the record an error occured to
     * @param errorType    The error type, such as {@link Event#ACK_TIMEOUT} or {@link
     *                     Event#MESSAGE_DENIED}
     * @param errorMessage An error message in english, describing the issue. Do not use this
     *                     message other than for logging! All checks should be against errorType
     */
    void onError(String recordName, Event errorType, String errorMessage);

    /**
     * Notified when a listener has confirmed that it will be publishing data. This is extremely
     * useful in the case of records, where you can indicate whether the values shown are cached or
     * up to date.
     *
