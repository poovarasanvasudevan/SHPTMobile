...
class UtilSingleNotifier implements UtilResubscribeNotifier.UtilResubscribeListener, UtilTimeoutListener {
    ...
    /**
     * Provides a scaffold for subscriptionless requests to io.deepstream.gherkin, such as the
     * SNAPSHOT and HAS functionality. The SingleNotifier multiplexes all the client requests so
     * that they can can be notified at once, and also includes reconnection funcionality incase the
     * connection drops.
     *
    ...
    /**
     * Process a response for a request. This overload of the method is for cases where data from
     * multiple messages has been merged into one deepstream message to save network traffic.
     *
     * Used in conjunction with {@link UtilSingleNotifier#request(String, String[],
     * UtilSingleNotifierCallback)}
     *
