...
/**
 * The main entry point for a DeepstreamClient. You can create a client directly using the
 * constructors or use the {@link DeepstreamFactory#getClient()}, {@link
 * DeepstreamFactory#getClient(String)} or {@link DeepstreamFactory#getClient(String, Properties)}
 * to create one for you and hold them for future reference.
 */
public class DeepstreamClient extends DeepstreamClientAbstract {

    /**
     * The getters for data-sync, such as {@link RecordHandler#getRecord(String)}, {@link
     * RecordHandler#getList(String)}, provider functionality such as {@link
     * RecordHandler#listen(String, ListenListener)} and single requests like {@link
     * RecordHandler#snapshot(String)}
     */
    public final  RecordHandler   record;
    /**
     * The entry point for events, such as {@link EventHandler#subscribe(String, EventListener)},
     * {@link EventHandler#emit(String)} and provider functionality such as {@link
     * EventHandler#listen(String, ListenListener)}
     */
    public final  EventHandler    event;
    /**
     * The entry point for rpcs, both requesting them via {@link RpcHandler#make(String, Object)}
     * and providing them via {@link RpcHandler#provide(String, RpcRequestedListener)}
     */
    public final  RpcHandler      rpc;
    /**
     * The entry point for presence, both querying for clients via {@link PresenceHandler#getAll()}
     * and subscribing/unsubscribing to login events via {@link PresenceHandler#subscribe(PresenceEventListener)}
     * and {@link PresenceHandler#unsubscribe(PresenceEventListener)}
     */
     ...
     *
     * @param url              URL to connect to. The protocol can be omited, e.g. <host>:<port>
     * @param deepstreamConfig A map of options that extend the ones specified in
    ...
    /**
     * Gets the {@link RecordHandler} used for data-sync in deepstream. Through records and lists
     * {@link RecordHandler#getRecord(String)}, {@link RecordHandler#getList(String)}, as well as
     * provider functionality such as {@link RecordHandler#listen(String, ListenListener)} and
     * single requests like {@link RecordHandler#snapshot(String)}
     *
    ...
    /**
     * Gets the {@link EventHandler} used for events in deepstream. Through subscribing and emitting
     * events {@link EventHandler#subscribe(String, EventListener)}, {@link
     * EventHandler#emit(String)}, as well as provider functionality such as {@link
     * EventHandler#listen(String, ListenListener)}
     *
    ...
    /**
     * Gets the {@link RpcHandler} used for rpcs in deepstream. Used for both requesting them via
     * {@link RpcHandler#make(String, Object)} and providing them via {@link
     * RpcHandler#provide(String, RpcRequestedListener)}
     */
    ...
    /**
     * Gets the {@link PresenceHandler} for presence functionality in deepstream. Both querying for
     * clients via {@link PresenceHandler#getAll()} and subscribing/unsubscribing to login events
     * via {@link PresenceHandler#subscribe(PresenceEventListener)} and {@link
     * PresenceHandler#unsubscribe(PresenceEventListener)}
     */
    ...
    /**
     * Adds a {@link DeepstreamRuntimeErrorHandler} that will catch all RuntimeErrors such as
     * AckTimeouts and allow the user to gracefully handle them.
     *
     ...
     *
     * Does not call the login callback, used mainly for anonymous logins where your guaranteed
     * login
     */
    ...
    /**
     * A callback that notifies the user if the login process was completed successfully or not, and
     * contains optional data received from the server associated to the user
     */
    interface LoginCallback {
         ...
         *
         * @param userData Optional data that is specific to the user and returned on succesfuly
         *                 authentication
         */
