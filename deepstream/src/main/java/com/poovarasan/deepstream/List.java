...
public class List {
    ...
    /**
     * Return whether the list has been destroyed. If true and you need to use the method create it
     * again via {@link RecordHandler#getList(String)} (String)}
     *
    ...
    /**
     * Discard the List. This should be called whenever you are done with the List retrieved by
     * {@link RecordHandler#getList(String)}. This does not guarantee that your subscriptions have
     * been unsubscribed, so make sure to do that first!<br/>
     *
     * If all usages of the same List have been discarded, the List will no longer be updated from
     * the server and any further usages will require the List to be retrieved again via {@link
     * RecordHandler#getList(String)}<br/>
     *
     * @return The list
     * @throws DeepstreamRecordDestroyedException Thrown if the list has been destroyed and can't
    ...
    /**
     * Notifies the user whenever the list has changed, and notifies immediately if triggerNow is
     * true
     *
     * @param listChangedListener The listener to add
    ...
    /**
     * Useful entry point for diffing previous list and new one to get entries added, removed and
     * moved
     */
