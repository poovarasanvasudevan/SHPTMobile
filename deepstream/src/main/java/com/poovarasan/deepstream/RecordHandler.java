...
public class RecordHandler {
    ...
    /**
     * Returns an anonymous record. A anonymous record is effectively a wrapper that mimicks the API
     * of a record, but allows for the underlying record to be swapped without losing subscriptions
     * etc.<br/>
     *
     * This is particularly useful when selecting from a number of similarly structured records.
     * E.g. a list of users that can be choosen from a list<br/>
     *
     * The only API differences to a normal record is an additional {@link
     * AnonymousRecord#setName(String)} method
     *
    ...
    /**
     * The following methods checks to prevent errors that occur when a record is discarded or
     * deleted and recreated before the discard / delete ack message is received.
     *
