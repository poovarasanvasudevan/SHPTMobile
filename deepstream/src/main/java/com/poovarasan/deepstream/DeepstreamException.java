...
/**
 * A DeepstreamException is a RuntimeException since they mostly occur when receiving an out of sync
 * message from the server, or if an Ack has timed out. You can catch these via {@link
 * DeepstreamClient#setRuntimeErrorHandler(DeepstreamRuntimeErrorHandler)} which would stop the
 * Error from being thrown. </br> The DeepstreamException extends {@link RuntimeException}
 */
