...
public class Record {
    ...
    /**
     * Return whether the record data has been destroyed. If true and you need to use the method
     * create it again via {@link RecordHandler#getRecord(String)}
     *
    ...
    /**
     * Remove listener added via {@link com.poovarasan.deepstream.Record#addRecordEventsListener(RecordEventsListener)}
     *
    ...
    /**
     * Set a merge strategy that comes with deepstream. These are currently LOCAL_WINS and
     * REMOTE_WINS
     *
     ...
     * a {@link com.poovarasan.deepstream.List}, but then you should always be using it via {@link
     * com.poovarasan.deepstream.List#getEntries()} ;)
     *
    ...
    /**
     * Set the value for the entire record<br/> Make sure that the Object passed in can be
     * serialised to a JsonElement, otherwise it will throw a {@link IllegalStateException}. Best
     * way to guarantee this is by setting Json friendly objects, such as {@link Map}. Since this is
     * a root the object should also not be a primitive.
     *
     ...
     * @return The record
     * @throws DeepstreamRecordDestroyedException Thrown if the record has been destroyed and can't
    ...
    /**
     * Set the value for the entire record synchronously and gives acknowledgement whether there
     * were any errors storing the record data in cache or storage.<br/> Make sure that the Object
     * passed in can be serialised to a JsonElement, otherwise it will throw a {@link
     * IllegalStateException}. Best way to guarantee this is by setting Json friendly objects, such
     * as {@link Map}. Since this is a root the object should also not be a primitive.
     *
     * @see Record#set(String, Object)
     */
     ...
     * @param triggerNow                Whether to immediately trigger the listener with the current
     *                                  value
     * @return The record
     * @throws DeepstreamRecordDestroyedException Thrown if the record has been destroyed and can't
     *                                            perform more actions
    ...
    /**
     * Notifies the user whenever anything inside the Record has changed, and triggers the listener
     * immediately.
     *
    ...
    /**
     * Remove the listener added via {@link Record#subscribe(String, RecordPathChangedCallback,
     * boolean)}
     *
    ...
    /**
     * Discard the record. This should be called whenever you are done with the record retrieved by
     * {@link RecordHandler#getRecord(String)}. This does not guarantee that your subscriptions have
     * been unsubscribed, so make sure to do that first!<br/>
     *
     * If all usages of the same record have been discarded, the record will no longer be updated
     * from the server and any further usages will require the record to be retrieved again via
     * {@link RecordHandler#getRecord(String)}<br/>
     *
     * Once the record is successfully discard, you can be notified via {@link
     * RecordEventsListener#onRecordDiscarded(String)}
     *
     * @return The record
     * @throws DeepstreamRecordDestroyedException Thrown if the record has been destroyed and can't
     *                                            perform more actions
    ...
    /**
     * Delete the record. This is called when you want to remove the record entirely from
     * deepstream, deleting it from storage and cache and telling all other users that it has been
     * deleted. This in turn will force all clients to discard the record.<br/>
     *
     * Once the record is successfully deleted, you can be notified via {@link
     * RecordEventsListener#onRecordDeleted(String)} (String)}</br>
     *
     * @return The record
     * @throws DeepstreamRecordDestroyedException Thrown if the record has been destroyed and can't
     *                                            perform more actions
    ...
    /**
     * Add a recordReadyListener as a callback. This means it will be called once when the record is
     * ready, either in sync or async if the record is not already ready.
     *
    ...
    /**
     * This gives us a handle to before and after a record is updated remotely. This is currently
     * used by {@link com.poovarasan.deepstream.List}
     *
     * @param recordRemoteUpdateHandler The listener to notify before and after an update is
     *                                  applied
     */
     ...
     * is directly after the clients. If no merge strategy is configure it will emit a
     * VERSION_EXISTS error and the record will remain in an inconsistent state.
     *
    ...
    /**
     * Generate a deep copy of the object and cast it to a class of any type, used by {@link
     * com.poovarasan.deepstream.List}
     */
    ...
    /**
     * This forces an update, which is useful when trying to reconcile a merge conflict when the
     * merge is the same but the version number isn't.
     */
    ...
    /**
     * Add a destroy pending listener, used by the RecordHandler and potentially other internal
     * stores
     */
    ...
    interface RecordDestroyPendingListener {
        /**
         * Called whenever the client is about to send the server a {@link Record#discard()} or
         * {@link Record#delete()} event.<br/> This should not be required to be implemented
         *
