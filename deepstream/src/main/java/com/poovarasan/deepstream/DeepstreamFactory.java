...
/**
 * A singleton to stop you ( the developer ) from having to pass around the deepstream client
 * reference around the codebase, making application development easier in frameworks such android.
 * <p> Currently this only contains a single deepstream client;
 */
public class DeepstreamFactory {
    ...
    /**
     * Returns the last client that was created. This is useful for most applications that only
     * require a single connection. The first time a client is connected however it has to be via
     * {@link DeepstreamFactory#getClient(String)} or {@link DeepstreamFactory#getClient(String,
     * Properties)}
     *
    ...
    /**
     * Returns a client that was previous created via the same url using this method or {@link
     * DeepstreamFactory#getClient(String, Properties)}. If one wasn't created, it creates it first
    ...
    /**
     * Returns a client that was previous created via the same url using this method or {@link
     * DeepstreamFactory#getClient(String)}. If one wasn't created, it creates it first and stores
